...
def test_card_init():
    card1 = PlayingCard("10", "Hearts")
    assert card1.rank == 3
    assert card1.suit == 1
    assert card1.trump is False

    card2 = PlayingCard(4, 2, True)
    assert card2.rank == 4
    assert card2.suit == 2
    assert card2.trump is True

    with pytest.raises(ValueError):
        PlayingCard("InvalidRank", "Diamonds")
    with pytest.raises(ValueError):
        PlayingCard("10", "InvalidSuit")


def test_card_get_rank():
    card1 = PlayingCard(0, 0)
    assert card1.get_rank() == "7"
    assert card1.get_rank(as_index=True) == 0

    card2 = PlayingCard(None, None)
...
def test_card_comparison():
    card1 = PlayingCard(0, 0)
    card2 = PlayingCard(0, 0)
    card3 = PlayingCard(1, 0)
    card4 = PlayingCard(0, 1)
    card5 = PlayingCard(0, 0, True)
